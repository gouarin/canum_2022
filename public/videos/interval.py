...
class Interval(Line):
    ...
    def __init__(
        self,
        level: int = 0,
        range: Sequence[int] = [0, 1],  # must be first
        ...
        **kwargs,
        ...
        if level < 0:
            raise "level must be positif"
        if len(range) != 2:
            raise "range must be of length 2"

        unit_size=1
        self.level = level
        self.i_range = range
        self.dx = 1./(1<<level)*unit_size

        if decimal_number_config is None:
            decimal_number_config = {
                "num_decimal_places": self._decimal_places_from_step(self.dx),
            }
    ...
    def add_cell_numbers(
        ...
        **kwargs,
        ...
        x = self.x_range[0] + 0.5*self.dx
        cell_numbers = VGroup()
        for i in range(*self.i_range):
            cell_numbers.add(
                self.get_cell_number_mobject(
                    i,
                    x,
                    font_size=font_size,
                    label_constructor=label_constructor,
